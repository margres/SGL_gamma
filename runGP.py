...
import io
#import sys
#sys.path.append('/home/grespanm/github/SLcosmological_parameters/GaPP/gapp/')
...
def plot_GP_result(self,zrec, hzrec, sighzrec ):
    ...
    if not os.path.exists(os.path.join(self.path_project, self.folder_figures)):
        print(f'making dir {os.path.join(self.path_project, self.folder_figure)}')
...
def run(self, plot_results=True):
    ...
    print(f'saving results GP as {os.paht.join(self.path_project, "hz_reconstructed_GP.txt")}')   
    # ========== saving the reconstructed hz 
    np.save(os.paht.join(self.path_project, "hz_reconstructed_GP.txt"),rec1)

    if plot_results:
        print('plotting the results')
        plot_GP_result(zrec, hzrec, sighzrec )
